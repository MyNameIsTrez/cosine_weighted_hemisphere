...
`ffmpeg -framerate 60 -i output/project/%06d.png -vf "fps=50,scale=960:-1:flags=lanczos,split[s0][s1];[s0]palettegen[p];[s1][p]paletteuse" -loop 0 output/output.gif`
...
Note that the `scale=960` setting here causes it to take a really long time to render, where the output makes it seem stuck printing `frame=0 fps=0.0` at the start, so you might want to lower that value.
